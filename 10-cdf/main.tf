resource "aws_cloudfront_distribution" "kalidindi" {
  enabled             = true
  ...
  records = [
    {
      ...
      type    = "A"
      alias   = {
        name    = aws_cloudfront_distribution.kalidindi.domain_name
        zone_id = aws_cloudfront_distribution.kalidindi.hosted_zone_id
      }
    }
  ]
}